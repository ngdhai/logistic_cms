...
final class Reflection
{
	 ...
	 * If the function has union or intersection type, it throws Nette\InvalidStateException or returns ReflectionType according to $asName.
	 * @return string|ReflectionType|null
	 */
	public static function getReturnType(\ReflectionFunctionAbstract $func, bool $asName = true)
	{
		$type = $func->getReturnType() ?? (PHP_VERSION_ID >= 80100 ? $func->getTentativeReturnType() : null);
		return self::getType($func, $type, $asName);
	}
	 ...
	 * If the parameter has union or intersection type, it throws Nette\InvalidStateException or returns ReflectionType according to $asName.
	 * @return string|ReflectionType|null
	 */
	public static function getParameterType(\ReflectionParameter $param, bool $asName = true)
	{
		return self::getType($param, $param->getType(), $asName);
	}
	 ...
	 * If the property has union or intersection type, it throws Nette\InvalidStateException or returns ReflectionType according to $asName.
	 * @return string|ReflectionType|null
	 */
	public static function getPropertyType(\ReflectionProperty $prop, bool $asName = true)
	{
		return self::getType($prop, PHP_VERSION_ID >= 70400 ? $prop->getType() : null, $asName);
	}
	 ...
	 */
	private static function getType($reflection, ?\ReflectionType $type, bool $asName = false)
	{
		if ($type === null) {
			return null;
			...
			return $asName
				? $name
				: new ReflectionType(($type->allowsNull() && $type->getName() !== 'mixed' ? '?' : '') . $name);
			...
			$names = array_map(
				function ($t) use ($reflection) { return self::normalizeType($t->getName(), $reflection); },
				$type->getTypes()
			);
